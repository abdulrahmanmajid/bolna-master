...
# Add the connection management code
...
class TaskManager(BaseManager):
        ...
        try:
            async with get_db_conn() as conn:
                result = await conn.fetchrow(
                    'SELECT timezone, country FROM agents WHERE agent_id = $1',
                    self.agent_id
                )
                if result:
                    return {
                        'timezone': result['timezone'] or 'America/Los_Angeles',
                        'country': result['country'] or 'US'
                    }
        except Exception as e:
            logger.error(f"Error loading agent config: {str(e)}")
        return {'timezone': 'America/Los_Angeles', 'country': 'US'}  # Default fallback

    async def initialize(self):
        try:
            async with get_db_conn() as conn:
                # Get agent config, prompts and template variables
                agent = await conn.fetchrow('''
                    SELECT agent_config, agent_prompts, template_variables, timezone, country 
                    FROM agents 
                    WHERE agent_id = $1
                ''', uuid.UUID(self.assistant_id))
                
                if agent:
                    self.agent_config = json.loads(agent['agent_config'])
                    self.agent_prompts = json.loads(agent['agent_prompts'])
                    self.template_variables = agent['template_variables']
                    timezone_str = agent['timezone'] or 'America/Los_Angeles'
                    self.timezone = pytz.timezone(timezone_str)
                    self.country = agent['country'] or 'US'
                    
                    # Merge template variables into config if they exist
                    if self.template_variables:
                        self.agent_config['template_variables'] = self.template_variables
        except Exception as e:
    ...
    async def load_prompt(self, assistant_name, task_id, local, **kwargs):
        ...
        # Load template variables early
        template_variables = await self._load_template_variables()
        if self.context_data is None:
            self.context_data = {}
        self.context_data['template_variables'] = template_variables
        ...
        if "system_prompt" in self.prompts:
            ...
            enriched_prompt = self._process_template(raw_prompt, self.context_data)
            ...
            self.system_prompt = {
                'role': "system",
                'content': f"{enriched_prompt}\n{notes}\n{DATE_PROMPT.format(today, current_time, self.timezone, self.country)}"
            }
        else:
            self.system_prompt = {
                'role': "system",
        ...
        if DATE_PROMPT in prompt_responses.get('task_1', {}).get('system_prompt', ''):
            date_info = DATE_PROMPT.format(today, current_time, str(self.timezone), self.country)
            prompt_responses['task_1']['system_prompt'] = prompt_responses['task_1']['system_prompt'].replace(DATE_PROMPT, date_info)
    ...
    def __prefill_prompts(self, task, prompt, task_type):
        if not prompt and task_type in ('extraction', 'summarization'):
            if task_type == 'extraction':
                ...
                prompt = EXTRACTION_PROMPT.format(today, current_time, self.timezone, self.country, extraction_json)
...
# Process template with Jinja2 {template_variables}

    def _process_template(self, template_str: str, context_data: Optional[Dict] = None) -> str:
        try:
            # Get template variables from context_data, default to empty dict if None
            template_variables = (context_data or {}).get('template_variables', {})
...
# Load Template variables with db and redis        

    async def _load_template_variables(self):
        try:
            # Check Redis first
            redis_key = f"template_variables:{self.agent_id}"
            temp_vars = await redis_client.get(redis_key)
            if temp_vars:
                return json.loads(temp_vars)
        
            # Fallback to database using connection pool
            async with get_db_conn() as conn:
                result = await conn.fetchrow(
                    'SELECT template_variables FROM agents WHERE agent_id = $1',
                    self.agent_id
                )
                if result and result['template_variables']:
                    return json.loads(result['template_variables'])
        except Exception as e:
...
# Print Conversation History and store transcrition to DB in JSON

    async def print_conversation_history(self, end_status="unknown"):
        ...
        conversation_json = {
            ...
            "metadata": {
                "turn_based": self.turn_based_conversation,
        ...
        # Store in Redis with call_sid as key
        if hasattr(self, 'call_sid'):
            redis_key = f"transcription:{self.call_sid}"
            try:
                await redis_client.set(redis_key, json.dumps(conversation_json), ex=3600)  # expire in 1 hour
                logger.info(f"Stored transcription in Redis for call_sid: {self.call_sid}")
            except Exception as e:
                logger.error(f"Failed to store transcription in Redis: {str(e)}")

        logger.info(json.dumps(conversation_json, indent=2))
        logger.info("==================================================")
