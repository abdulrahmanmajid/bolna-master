...
class ConversationConfig(BaseModel):
    ...
    trigger_user_online_message_after: Optional[int] = 50
...
class AgentModel(BaseModel):
    ...
    call_direction: str = Field(..., pattern="^(inbound|outbound)$")
    ...
    @field_validator('inbound_phone_number')
    def validate_inbound_phone_number(cls, v, values):
        if v is not None and values.data.get('call_direction') == 'outbound':
            raise ValueError('inbound_phone_number cannot be set for outbound agents')
        return v
